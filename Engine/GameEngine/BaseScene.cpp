...
void BaseScene::loadScene() {
    ...
    try {
        ...
        for (const auto& model : modelData) {
            ...
            if (name == "player") {
                pObject = new Player();
                dynamic_cast<Player*>(pObject)->mObjectsToDraw = &mObjectsToDraw;

                pObject->shapeType = AABB;
                pObject->pShape = new sPhysicsProperties::sAABB(pObject->getBoundingBoxMin(), pObject->getBoundingBoxMax());
            }
            else {
                pObject = new GameObject(file);
